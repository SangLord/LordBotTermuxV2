...
* REMAKE OF FADHIL GRAPHY × BRYAN GAY:V × SANG LORD
...
const { help } = require('./Lord/help')
const { rules } = require('./Lord/rules')
const { bahasa } = require('./Lord/bahasa')
const { donasi } = require('./Lord/donasi')
const { limits } = require('./Lord/limit')
const { bokep } = require('./Lord/bokep.js')
const { ownermenu } = require('./Lord/ownermenu.js')
const { groupmenu } = require('./Lord/groupmenu.js')
const { mediamenu } = require('./Lord/mediamenu.js')
const { nsfwmenu } = require('./Lord/nsfwmenu.js')
const { listmenu } = require('./Lord/listmenu.js')
const { funmenu } = require('./Lord/funmenu.js')
const { makermenu } = require('./Lord/makermenu.js')
//jangan hiraukan ini!!,,,ini uji coba yang gagal:) kontol//
//const { gabutmenu } = require('./Lord/gabutmenu')
//const { ownermenu } = require('./Lord/ownermenu')
//const { groupmenu } = require('./Lord/groupmenu')
//const { vipmenu } = require('./Lord/vipmenu')
//const { nsfwmenu } = require('./Lord/nsfwmenu')
//const { creatormenu } = require('./Lord/creatormenu')
//const { downloadmenu } = require('./Lord/downloadmenu')
//const { allmenu } = require('./Lord/allmenu')
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('BURUAN SCAN!! MADED BY SANG LORD'))
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const Lord = ["6283144780782@s.whatsapp.net"]
			...
			mess = {
				...
				only: {
					...
					userB: `[ ⚠️UNREGISTED⚠]\n\n️Hai Kak *${pushname2}* Kamu Belum Menjadi Teman *LordBot* \nSilahlan Daftar Dulu Yaa Dengan Cara\n\n Ketik ${prefix}daftar Lord/17`,
		...
		case 'antilink':
			...
			case 'addbadword':
             ...
             case 'nobadword':
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
                if (args.length < 1) return reply('lelЁЯЧ┐')
                if (args[0] === 'enable') {
		...
		case 'daftar':
            ...
            if (!isUser) return reply(mess.only.userB)
                ...
                const tanggal = `${thisDay}, ${day} - ${myMonths[bulan]} - ${year}`
					await costum(help(prefix, instagram, yt, name, pushname2, user, limitt), text, FarhanGans, rmenu)
    				break 
/*********PISAH MENU**********/
              case 'ownermenu':
					if (isBanned) return reply(mess.only.benned)
		    ...
		    case 'map':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *SangLordBotz BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu!*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[SangLordBotz]','Yellow'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
